...
This is my data visualization portfolio, aka. the ultimate copy and paste tool.

The whole markdown should run by iteself given all the packages are installed and you download the .Rdata file.

Below is the list of all the required packages. Mostly graphical tools, along with some statistical analysis resources. Any non-trivial function call should have the corresponding package specified.
...
# Simulate data for a barplot
dataBarplot <- data.frame(
  Cancer = rep(c("Cervix", "Vulva", "Vagina", "Penis", "Anus", "Oropharynx"), each = 4),
  Stage = factor(rep(c("High grade lesions", "Early stage cancer", "Intermediate stage cancer", "Advanced stage cancer"), 6),
                levels = c("High grade lesions", "Early stage cancer", "Intermediate stage cancer", "Advanced stage cancer")),
  Cost = 5000) %>%
  mutate(
    Cost = ifelse(Cancer == "Cervix", Cost - runif(1, 1000, 2000), Cost),
    Cost = ifelse(Cancer == "Vulva", Cost - runif(1, 1000, 3000), Cost),
    Cost = ifelse(Cancer == "Vagina", Cost + runif(1, -1000, 2000), Cost),
    Cost = ifelse(Cancer == "Penis", Cost + runif(1, 0, 2000), Cost),
    Cost = ifelse(Cancer == "Anus", Cost + runif(1, 0, 3000), Cost),
    Cost = ifelse(Cancer == "Oropharynx", Cost + runif(1, 0, 5000), Cost),
    
    Cost = ifelse(Stage == "High grade lesions", Cost + runif(1, 2000, 7000), Cost),
    Cost = ifelse(Stage == "Early stage cancer", Cost + runif(1, 0, 3000), Cost),
    Cost = ifelse(Stage == "Intermediate stage cancer", Cost - runif(1, 0, 4000), Cost),
    Cost = ifelse(Stage == "Advanced stage cancer", Cost - runif(1, 0, 5000), Cost),
   Cost = Cost + 3000
  )
...
ggplot(dataBarplot) + 
  # Add a stacked bar chart (geom_col creates bars with heights determined by 'N')
  geom_col(
    mapping = aes(reorder(Cancer, -Cost), Cost, fill = Stage), # Map 'N' to x, 'Symptome' to y, and 'Temps' to fill color
    width = 0.6,                              # Set the bar width
    position = "stack"                        # Stack bars by the 'fill' variable
  ) +
  # Add a vertical line at x = 0 for reference
  ...
  # Customize the x-axis scale
  scale_y_continuous(
    limits = c(0, 50000),                     # Set the range for the x-axis
    breaks = seq(0, 50000, 10000),             # Major tick marks every 1000 units
    minor_breaks = seq(5000, 45000, 10000),      # Minor tick marks every 500 units
    expand = c(0, 0)                         # Remove padding on the x-axis
  ) + 
  scale_fill_manual(
    name = "Cancer stage",
    values = c('#28AFB0',  "#B2FF9E", '#F6803C',  '#82354F')
  ) +
  xlab("Potentially HPV-related disease") +
  ylab("HCRU costs (euros)") +
  # Customize the overall appearance of the plot
  theme(
    panel.background = element_rect(fill = "white"),   # Set panel background to white
    panel.grid.major.y = element_line(color = "darkgray"), # Major grid lines for x-axis
...
dataBoxplot <- data.frame(
  TIME = factor(
      rep(rep(c("T0", "T3", "T6", "T9", "T12", "T15", "T18", "T24"), each = 100), 2),
      levels = c("T0", "T3", "T6", "T9", "T12", "T15", "T18", "T24")),
  Score = c(rnorm(100, 10, 1), rnorm(100, 12, 2), rnorm(100, 13, 2), rnorm(100, 13, 1),
            rnorm(100, 14, 2), rnorm(100, 15, 1), rnorm(100, 17, 1), rnorm(100, 18, 3),
            rnorm(100, 10, 3), rnorm(100, 11, 1), rnorm(100, 11, 2), rnorm(100, 12, 1),
            rnorm(100, 14, 1), rnorm(100, 16, 3), rnorm(100, 15, 1), rnorm(100, 15, 4)),
  Cohort = factor(rep(c("First Group", "Second Group"), each = 8*100),
                  c("First Group", "Second Group"))
)

ggplot(dataBoxplot) +
  # Add a boxplot layer for setup scores grouped by operation ID
  geom_boxplot(
    aes(group = interaction(TIME, Cohort), 
        x = TIME, 
        y = Score, 
        colour = Cohort),  
  ...
  annotate(
    "text",
    x = 1:nlevels(dataBoxplot$TIME)-.2, y = 27.5,
    label = paste0("n=", table(dataBoxplot$TIME[dataBoxplot$Cohort=="Second Group"])),
    angle = 65, hjust = 0
  ) +
  annotate(
    "text",
    x = 1:nlevels(dataBoxplot$TIME)+.2, y = 27.5,
    label = paste0("n=", table(dataBoxplot$TIME[dataBoxplot$Cohort=="First Group"])),
    angle = 65, hjust = 0
  ) +
  ylab("Score value") +
  # Label the x-axis
  ...
  # Customize the y-axis scale
  scale_y_continuous(
    limits = c(0, 40),         # Set y-axis limits between 0 and 10
    breaks = seq(0, 30, by = 5),             # Add breaks at each integer value
    expand = c(0, 0)           # Remove padding on the axis
  ) +
  # Customize the x-axis scale
  scale_x_discrete(
    labels = c("T0", "T3", "T6", "T9", "T12", "T15", "T18", "T24")
  ) + 
  scale_color_manual(
    values = c("First Group" = "#005263", "Second Group" = "#E64E2B")
  ) +
  # Apply a minimal theme for clean visualization
  theme_minimal()
```
...
p1 <- ggplot(plotData) +
  # Add a boxplot layer for setup scores grouped by operation ID
  geom_histogram(
    aes(x = val),# Map operation ID and setup scores to boxplot
    fill = "white",
    color = "black",
    closed = "left",
    binwidth = 1
  ...
  # Customize the y-axis scale
  scale_x_continuous(
    limits = c(0, 15),         # Set y-axis limits between 0 and 10
    breaks = 0:15,             # Add breaks at each integer value
    expand = c(0, 0)# Remove padding on the axis
  ) +
  # Apply a minimal theme for clean visualization
  theme_minimal() +
...
p2 <- ggplot(plotData) +
  ...
  scale_x_continuous(
    ...
    breaks = 0:15,             # Add breaks at each integer value
    expand = c(0, 0) ,
    position = "top" # Remove padding on the axis
  ) +
  scale_y_continuous(
    breaks = 0,
    labels = 10
...
```{r}
dataRadarTemp <- data.frame(
  TIME = rep(c("T0", "T3", "T6", "T9"), each = 100),
  MO = c(runif(100, 1, 3), runif(100, 2, 5), runif(100, 1, 4), runif(100, 0, 2)),
  SC = c(runif(100, 1, 4), runif(100, 0, 3), runif(100, 2, 5), runif(100, 2, 2.1)),
  UA = c(runif(100, 2, 4), runif(100, 4, 5), runif(100, 4, 5), runif(100, 4, 5)),
  PA = c(runif(100, 2, 2.5), runif(100, 3, 4), runif(100, 3, 5), runif(100, 4, 5)),
  AD = c(runif(100, 1, 3), runif(100, 1, 3), runif(100, 1, 3), runif(100, 1, 3))
) %>%
  mutate(
    TIME = factor(
      TIME,
      levels = c("T0", "T3", "T6", "T9"),
      labels = c("T0", "T3", "T6", "T9")
    )
  )

# Average by dimension and time point
dataRadar <- data.frame(
  MO = by(dataRadarTemp$MO, dataRadarTemp$TIME, mean),
  SC = by(dataRadarTemp$SC, dataRadarTemp$TIME, mean),
  UA = by(dataRadarTemp$UA, dataRadarTemp$TIME, mean),
  PA = by(dataRadarTemp$PA, dataRadarTemp$TIME, mean),
  AD = by(dataRadarTemp$AD, dataRadarTemp$TIME, mean)
)

# Adding a first line with the max and a second line with the min
# Mandatory to use the function
dataRadar <- rbind(rep(5,5), rep(0,5) , dataRadar)


# Color vector
# Converting colors in rgb, with alpha for trasnparency
alpha1 <- 1*255
alpha2 <- 0*255
colors_border <- c(rgb(0, 82, 99, alpha1, maxColorValue = 255),
                  rgb(125,175,140, alpha1, maxColorValue = 255),
                  rgb(152,202,139, alpha1, maxColorValue = 255),
                  rgb(193,229,227, alpha1, maxColorValue = 255))
colors_in <- c(rgb(0, 82, 99, alpha2, maxColorValue = 255),
                rgb(125,175,140, alpha2, maxColorValue = 255),
                rgb(152,202,139, alpha2, maxColorValue = 255),
               rgb(193,229,227, alpha2, maxColorValue = 255))
...
fmsb::radarchart(
  ...
  cglty = 1,
  axislabcol = "grey",
  caxislabels = seq(3, 5, .5),
  cglwd = 0.8,
  #custom labels
  vlcex = 0.8,
  vlabels = c("Mobility", "Selfcare", "Usual\nActivity", "Pain", "Anxiety\nDepression")
)
```
...
ggplot2::autoplot(fit, surv.linetype = 'blank', surv.colour = NA) +  # Start with a blank base plot
...
ggplot2::autoplot(fit, surv.linetype = 'blank', surv.colour = NA) + 
...
```{r}
data <- data.frame(
  time = c(rnorm(90, 180, 60),rnorm(90, 130, 60)),
  event = rbinom(180, 0:1, 0.7),
  group = rep(c("MONO", "COMBO"), each = 90)
)


get_median_time <- function(dataSurv, lab) {
  data <- dataSurv[dataSurv$group == lab,]
  as.numeric(surv_median(surv_fit(
    Surv(time = time, event = event) ~ 1, 
    data = data
  ))[2])
}
median1 <- get_median_time(data, "MONO")
median2 <- get_median_time(data, "COMBO")
...
ggsurvfit::survfit2(
    ...
    annotate("segment", 
           x = 0, xend = median2, 
            ...
            colour = "gray70", lty = 2) +
    annotate("segment", 
             x = median1, xend = median1, 
             y = .5, yend = 0, 
             colour = "gray70", lty = 2) +
    annotate("segment", 
             x = median2, xend = median2, 
             y = .5, yend = 0, 
             colour = "gray70", lty = 2) +
    labs(
      x = "Time in days",
      y = "Probability of suvival"
    )
```
...
couts <- TraMineR::seqsubm(seqCarpet, method = "CONSTANT", cval = 2)

# Compute optimal matching distances
seq.om <- TraMineR::seqdist(seqCarpet,
...
ggpubr::ggarrange(
  ggseqplot::ggseqiplot(seqCarpet, facet_ncol = 1, no.n = TRUE) +  # Individual plot
    theme(
  ...
  ggseqplot::ggseqiplot(  # Plot grouped sequences
    seqCarpet,
...
load("C:/Users/hugo.marthinet/Downloads/index.Rdata")
...
bodyMap <- cartogram::cartogram_cont(
